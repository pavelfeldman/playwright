 ...
 */
 function wrapText(text, options, prefix) {
  ...
  for (const line of lines)
    result.push(wrapLine(line, options?.maxColumns, prefix, !result.length));
  return result.join('\n');
}
 ...
 * @returns {string}
 */
function wrapLine(textLine, maxColumns, prefix, isFirstLine) {
  if (!maxColumns)
  ...
  for (const word of words) {
    if (line.length && line.length + word.length < maxColumns) {
      ...
      if (line)
        lines.push(line);
      line = (isFirstLine && lines.length ? indent : prefix) + word;
    }
  }
  if (line)
    lines.push(line);
