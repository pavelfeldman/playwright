...
export class LocalSocksProxy implements SocksConnectionClient {
  ...
  setLocalAddress(uid: string, host: string, port: number) {
    ...
    if (connection)
      connection.setLocalAddress(host, port);
  }

  async onSocketRequested(uid: string, host: string, port: number): Promise<void> {
    try {
      const { address } = await dnsLookupAsync(host);
      const socket = await createSocket(address, port);
      socket.on('error', () => this.sendError(uid, ''));
      socket.on('data', data => this.sendData(uid, data));
      socket.on('end', () => this.sendEnd(uid));
      const localAddress = socket.localAddress;
      const localPort = socket.localPort;
      this._sockets.set(uid, socket);
      this.setLocalAddress(uid, localAddress, localPort);
    } catch (error) {
      this.sendError(uid, error.code);
    }
  }
  ...
  protected sendData(uid: string, data: Buffer) {
    this._connections.get(uid)?.sendData(data);
  }

  protected sendError(uid: string, code: string) {
    this._connections.get(uid)?.sendError(code);
  }

  protected sendEnd(uid: string) {
    this._connections.get(uid)?.sendEnd();
