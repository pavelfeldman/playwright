...
export const SplitView: React.FC<SplitViewProps> = ({
  ...
  return <div className='split-view'>
    <div className='split-view-main'>{children[0]}</div>
    <div style={{flexBasis: size}} className='split-view-sidebar'>{children[1]}</div>
    <div
      style={{bottom: resizing ? 0 : size - 32, top: resizing ? 0 : undefined, height: resizing ? 'initial' : 32 }}
      className='split-view-resizer'
      onMouseDown={event => { console.log(event.clientY); setResizing({ offsetY: event.clientY - (event.target as HTMLElement).getBoundingClientRect().y })}}
