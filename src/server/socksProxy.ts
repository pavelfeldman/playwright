...
export interface ProtocolApi {
  on(event: 'Tethering.socketRequested', listener: (params: { uid: string, host: string, port: number }) => void): this;
  on(event: 'Tethering.socketData', listener: (params: { uid: string, data: Buffer }) => void): this;
  on(event: 'Tethering.socketClosed', listener: (params: { uid: string }) => void): this;
  emit(event: 'Tethering.socketRequested', params: { uid: string, host: string, port: number }): boolean;
  emit(event: 'Tethering.socketData', params: { uid: string, data: Buffer }): boolean;
  emit(event: 'Tethering.socketClosed', params: { uid: string }): boolean;

  setLocalAddress(uid: string, host: string, port: number): void;
  sendData(uid: string, data: Buffer): void;
  sendError(uid: string, code: string): void;
  sendEnd(uid: string): void;
}
...
class Connection extends EventEmitter {
  ...
  private _protocolApi: ProtocolApi;
  private _uid: string;

  constructor(uid: string, socket: net.Socket, proxy: SocksProxy) {
    super();
    this._uid = uid;
    this._socket = socket;
    this._protocolApi = proxy;
  ...
  async _run() {
    ...
    this._protocolApi.emit('Tethering.socketRequested', { uid: this._uid, host, port });
  }
  ...
  private _onClose() {
    this._protocolApi.emit('Tethering.socketClosed', { uid: this._uid });
    this.emit('close');
  }
  ...
  _setLocalAddress(host: string, port: number) {
    this._writeBytes(Buffer.from([
      0x05,
    ...
    this._socket.on('data', data => this._protocolApi.emit('Tethering.socketData', { uid: this._uid, data }));
  }

  _sendError(code: string) {
    const buffer = Buffer.from([0x05, SocksReply.GeneralServerFailure]);
    switch (code) {
      case 'ENOENT':
      case 'ENOTFOUND':
      case 'ETIMEDOUT':
...
export class SocksProxy extends EventEmitter implements ProtocolApi {
  ...
  constructor() {
    ...
    this._server = new net.Server((socket: net.Socket) => {
      const uid = helper.guid();
      const connection = new Connection(uid, socket, this);
  ...
  setLocalAddress(uid: string, host: string, port: number) {
    const connection = this._connections.get(uid);
    if (connection)
      connection._setLocalAddress(host, port);
  }

  // Protocol handler.
  sendData(uid: string, data: Buffer) {
    const connection = this._connections.get(uid);
    if (connection)
      connection._sendData(data);
  }

  sendError(uid: string, code: string) {
    const connection = this._connections.get(uid);
    if (connection)
      connection._sendError(code);
  }
