...
import { assert, headersArrayToObject, headersObjectToArray } from '../../utils/utils';
...
export class WKPage implements PageDelegate {
  ...
  pdf?: ((options?: types.PDFOptions | undefined) => Promise<Buffer>) | undefined;
  coverage?: (() => any) | undefined;
  cspErrorsAsynchronousForInlineScipts?: boolean | undefined;
  ...
  private _onExecutionContextCreated(contextPayload: Protocol.Runtime.ExecutionContextDescription) {
    if (this._contextIdToContext.has(contextPayload.id))
      return;
    const frame = this._page._frameManager.frame(contextPayload.frameId) || this._page.mainFrame();
  ...
  async navigateFrame(frame: frames.Frame, url: string, referrer: string | undefined): Promise<frames.GotoResult> {
    ...
    return { newDocumentId: 'document-id-1' };
  }
