...
export class RecorderApp extends EventEmitter {
  ...
  async onCallLog(log: string[]): Promise<void> {
    await this._page.mainFrame()._evaluateExpression(((log: string[]) => {
      window.playwrightSetLog(log);
    }).toString(), true, log, 'main').catch(() => {});
  }
