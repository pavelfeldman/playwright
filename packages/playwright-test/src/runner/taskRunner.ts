...
export class TaskRunner<Context> {
  ...
  async run(context: Context, deadline: number, interruptPromise?: ManualPromise<void>): Promise<FullResult['status']> {
    const { status, cleanup } = await this.runDeferCleanup(context, deadline, interruptPromise);
    const teardownStatus = await cleanup();
    return status === 'passed' ? teardownStatus : status;
  }

  async runDeferCleanup(context: Context, deadline: number, interruptPromise = new ManualPromise<void>()): Promise<{ status: FullResult['status'], cleanup: () => Promise<FullResult['status']> }> {
    ...
    if (sigintWatcher.hadSignal() || interruptPromise?.isDone()) {
      status = 'interrupted';
