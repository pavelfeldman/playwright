...
import type { JsonConfig, JsonProject, JsonSuite, JsonTestCase, JsonTestResultEnd, JsonTestResultStart, JsonTestStepEnd, JsonTestStepStart } from '../../../trace-viewer/src/teleReceiver';
...
export class TeleReporterEmitter implements Reporter {
  ...
  private _onStdIO(type: 'stdio' | 'stderr', chunk: string | Buffer, test: void | TestCase, result: void | TestResult): void {
    ...
    const data = typeof isBase64 ? chunk : chunk.toString('base64');
    this._messageSink({
      method: 'onStdIO',
  ...
  private _serializeProject(suite: Suite): JsonProject {
    ...
    const report: JsonProject = {
      id: (project as any)._internal.id,
