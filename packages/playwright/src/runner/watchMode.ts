...
export async function runWatchModeLoop(configLocation: ConfigLocation, initialOptions: WatchModeOptions): Promise<reporterTypes.FullResult['status']> {
  ...
  testServerConnection.onRecoverFromStepError(({ message, location }) => {
    ...
    readKeyPress(text => {
      if (text === 'c') {
        ...
        testServerConnection.resumeAfterStepError({ disposition: 'continue' }).catch(() => {});
      } else if (text === 't') {
        process.stdout.write(`\n${colors.dim('Throwing error...')}\n`);
        testServerConnection.resumeAfterStepError({ disposition: 'throw' }).catch(() => {});
      }
      return text;
    });
  });
